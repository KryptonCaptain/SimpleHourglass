...
import vazkii.s_hg.api.internal.VanillaPacketDispatcher;
...
public class BlockHourglass extends BlockModContainer implements IWandHUD {
    ...
    */
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xs, float ys, float zs) {
		...
		if(stack == null && player.isSneaking()) {
				hourglass.lock = !hourglass.lock;
				return false;
		}
				
		if(hourglass.lock) {
			if(!player.worldObj.isRemote)
				player.addChatMessage(new ChatComponentTranslation("s_hg_misc.hourglassLock"));
			return true;
		}
